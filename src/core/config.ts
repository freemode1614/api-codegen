 ...
 */
async function loadFromFile(filePath: string): Promise<Partial<ApicodegenConfig>> {
 ...
 */
function validateConfig(config: Partial<ApicodegenConfig>): config is ApicodegenConfig {
  if (!config.spec) {
 ...
 */
export function configToCLIOptions(config: ApicodegenConfig): Record<string, unknown> {
