  ...
  private toBaseSchema(
    ...
    if (this.isOpenAPIArraySchema(schema)) {
      ...
      return {
        ...
        allOf: allOf?.map((s) => (this.isRef(s) ? s : this.toBaseSchema(s))),
        anyOf: anyOf?.map((s) => (this.isRef(s) ? s : this.toBaseSchema(s))),
        oneOf: oneOf?.map((s) => (this.isRef(s) ? s : this.toBaseSchema(s))),
        properties: Object.keys(properties).reduce((acc, p) => {
          ...
          return {
            ...
            [p]: {
              ...
              ref: this.isRef(propSchema) ? propSchema.$ref : undefined,
            },
          };
        }, {}),
      };
